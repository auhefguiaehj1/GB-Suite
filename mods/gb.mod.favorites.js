    ...
    window.GBSuite.register('Favorites', (function () {
        ...
        (function addStyles() {
            ...
            s.textContent = `
                ...
                min-width:22px; min-height:22px; padding:2px 6px;
                border-radius:6px; background:rgba(255,105,180,.9);
                color:#fff; font-size:14px; font-weight:700; line-height:1;
                font-family:system-ui,-apple-system,"Segoe UI","Segoe UI Symbol","Apple Color Emoji","Noto Color Emoji",sans-serif;
                display:flex; align-items:center; justify-content:center;
                pointer-events:none; box-shadow:0 2px 6px rgba(0,0,0,.25);
                }
            `;
        ...
        function ensureBadge(anchor) {
            ...
            // Host = bestehender anim-Wrapper oder unser generischer Overlay-Wrapper
            let host = img.closest('.gb-anim-wrap') || img.closest('.gb-overlay-wrap');
            if (!host) {
                host = document.createElement('span');
                host.className = 'gb-overlay-wrap';
                // vorhandenen Radius übernehmen, damit alles sauber clippt
                const br = getComputedStyle(img).borderRadius;
                if (br && br !== '0px') host.style.borderRadius = br;
                img.style.display = 'block';           // Baseline-Gap verhindern
                img.parentNode.insertBefore(host, img);
                host.appendChild(img);
            }

            // Badge innerhalb des Hosts platzieren (immer oben rechts IM Bild)
            let badge = host.querySelector('.gb-fav-badge');
            if (!badge) {
                badge = document.createElement('div');
                badge.className = 'gb-fav-badge';
                badge.textContent = '♥';               // reines Herzzeichen
