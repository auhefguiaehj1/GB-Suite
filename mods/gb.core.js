(function () {
  ...
  // Styles (gekürzt – ident zu deiner Version)
  addStyle(`
  ...
  // Settings-Link in die Sub-Navi (neben „Redeem Code“)
  function mountSettingsButton(){
    const navSub = document.querySelector('.navSubmenu');
    if (!navSub) return;
    if (navSub.querySelector('.gb-suite-link')) return; // keine Duplikate
  ...
  window.GBSuite = {
    ...
    start(){
      const run = () => {
        ...
        mountSettingsButton();          // <- Settings-Link
        Registry.initEnabled();         // <- startet alle Module (Blacklist, TagGroups, Favorites, InfiniteScroll …)
      };
